...
void ofApp::draw(){
	...
	ofRotateZ(180);
	ofRotateY(180);
	ofTranslate(ofVec3f(-this->frame_img.getWidth() / 2, -this->frame_img.getHeight() / 2,  -1024));
	...
	for (int y = 0; y < this->depth.rows; y += 5) {
		for (int x = 0; x < this->depth.cols; x += 5) {
			...
			if (z != 0 && z < 1024) {
				locations.push_back(ofVec3f(x -ofGetWidth() / 2, y - ofGetHeight() / 2, z - 1024));
			}
		}
	}
	...
	for (int i = 0; i < locations.size(); i++) {
		...
		for (int j = 0; j < locations.size(); j++) {
			if(i != j){
				float distance = locations[i].distance(locations[j]);
				if (distance <= 8) {
					ofColor c;
					c.setHsb(ofNoise(locations[i] * 0.005) * 255, 255, 255);
					ofSetColor(c);
					ofDrawSphere(locations[i], 1);
